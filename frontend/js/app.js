// BDO Optima — app.js (v4: building-aware bypass)
document.addEventListener('DOMContentLoaded', () => {
    ...
    let leafletMap = null;
    let buildingLayer = null;   // Leaflet bina poligon grubu
    let currentResult = null;
    let currentViewMode = 'after';

    console.log("[BDO] app.js v4 (building-aware)");
    ...
    function activeTrafficData() {
        if (!currentResult || !currentResult.traffic) return null;
        const t = currentResult.traffic;
    ...
    function updateStats(result) {
        ...
        const cmp = result.traffic?.comparison;
        if (cmp) {
    ...
    function buildRequestUrl() {
        ...
        if (dataSource.value === 'osm') {
            ...
            return {
                url: `http://127.0.0.1:8000/generate_osm?lat=${lat}&lon=${lon}&radius=${radius}&trips=${trips}`,
            ...
            return {
                url: `http://127.0.0.1:8000/generate?iterations=${iter}&snapping=${snap}&trips=${trips}`,
                label: `L-System (iter=${iter}, snap=${snap})`,
            };
        }
    }
        ...
        runBtn.addEventListener('click', async () => {
            ...
            loader.innerHTML = `
                ...
                <p style="font-size:0.7rem;opacity:0.5;margin-top:0.4rem;">
                    Yol + Bina → Simülasyon → Geometrik Filtre → AI → Tekrar Sim
                </p>`;
            ...
            try {
                ...
                currentViewMode = result.traffic?.after ? 'after' : 'before';

                const radios = document.querySelectorAll('input[name="viewMode"]');
                radios.forEach(r => {
                    r.checked = (r.value === currentViewMode);
                    r.disabled = (r.value === 'after' && !result.traffic?.after);
                });
                ...
                const before = result.traffic?.before;
                if (before) {
                    log(`[SIM] Önce: max ${before.max_load}, ort ${before.avg_load}`, "info");
                }

                const aiSel = result.ai_selection;
                if (aiSel) {
                    log(`[GEOM] ${aiSel.candidates_count} geçerli aday üretildi`, "info");
                    if (aiSel.chosen) {
                        log(`[AI] Seçim: ${aiSel.chosen.from} ↔ ${aiSel.chosen.to}`, "success");
                    }
                }
