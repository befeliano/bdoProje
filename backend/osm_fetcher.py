...
def _build_combined_query(lat: float, lon: float, radius_m: int) -> str:
    """
    Yol ağını, binaları ve trafik sinyallerini TEK sorguda çek.
    Üç ayrı sorgudan ~3× hızlı.

    Overpass syntax notu:
        - 'out body;' way'lerin tag'lerini ve node referans listesini döndürür
        - '>;' way'lerin içindeki node'ları çek (recurse down)
        - 'out skel qt;' bu çekilen node'ların koordinatlarını verir (tag yok)
        - Sinyal node'ları için ayrı 'out body' (tag'leri korunsun diye)
    """
    highway_regex = "|".join(NETWORK.highway_types)
    return f"""
    [out:json][timeout:{OVERPASS.timeout_seconds}];
    (
      // 1. Araç yolları (tag'leri ile)
      way(around:{radius_m},{lat},{lon})["highway"~"^({highway_regex})$"];
      // 2. Binalar
      way(around:{radius_m},{lat},{lon})["building"];
    )->.ways;
    (
      // 3. Trafik sinyali node'ları (tag'leri ile, ayrı set)
      node(around:{radius_m},{lat},{lon})["highway"="traffic_signals"];
    )->.signals;

    // Way'lerin tag ve node referanslarını yaz
    .ways out body;
    // Way'lerin içindeki node'ları çöz, sadece koordinat olarak yaz
    .ways >;
    out skel qt;
...
def _split_overpass_elements(elements: list) -> tuple[
    dict[int, OsmNode], list[OsmWay], list[OsmWay], set[int]
]:
    """
    ...
    return osm_nodes, highway_ways, building_ways, signal_node_ids
...
def fetch_osm_area(
    ...
    osm_nodes, highway_ways, building_ways, signal_ids = _split_overpass_elements(elements)
    logger.info("Çiğ veri: %d node, %d yol, %d bina, %d sinyal",
                len(osm_nodes), len(highway_ways), len(building_ways), len(signal_ids))
    ...
    # TOON içindeki düğüm/kenar sayısını çıkar
    n_nodes = sum(1 for ln in toon.split("\n") if ln.startswith("NODE;"))
    n_edges = sum(1 for ln in toon.split("\n") if ln.startswith("EDGE;"))

    logger.info("Bitti: %d kavşak, %d kenar, %d bina (%.2fs)",
                n_nodes, n_edges, buildings["count"], fetch_time)
